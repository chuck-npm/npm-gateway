...
use NpmGateway\Database\Migration\OperationsCategorySchema as Schema;
return new class implements MigrationInterface {
 public function up(mysqli $db):void
 {
  $this->assertTable($db);$this->assertConstraint($db,Schema::BEFORE_CATEGORIES);
  if($this->count($db,"SELECT COUNT(*) FROM user_category_access WHERE category NOT IN ('finance','human-resources','company-notices','marketing','admin','credit-cards')")>0)throw new MigrationException('Unsupported category rows prevent Operations migration.');
 ...
 public function down(mysqli $db):void
 {
  ...
  if($this->count($db,"SELECT COUNT(*) FROM user_category_access WHERE category='operations'")>0)throw new MigrationException('Cannot roll back Operations while Operations authorization memberships exist.');

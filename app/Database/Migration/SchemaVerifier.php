...
final class SchemaVerifier
{
    ...
    public function verify(): array
    {
        ...
        if(isset($executed[SupplyOrdersSchema::MIGRATION])){$this->verifySupplyOrdersSchema();}elseif(isset($executed[RmAuditsSchema::MIGRATION])){$this->verifyRmAuditsSchema();}elseif(isset($executed[RmCorrectionsSchema::MIGRATION])){$this->verifyRmCorrectionsSchema();}elseif(isset($executed[ApplicationReviewsCategorySchema::MIGRATION])){$this->verifyApplicationReviewsCategorySchema();}elseif(isset($executed[OperationsCategorySchema::MIGRATION])){$this->verifyOperationsCategorySchema();}elseif(isset($executed[CompanyNoticesCategorySchema::MIGRATION])){$this->verifyCompanyNoticesCategorySchema();}
    ...
    private function verifyRmAuditsSchema():void
    {
        $this->assertCheckValues('user_category_access','chk_user_category_access_category',['operations','finance','human-resources','company-notices','application-reviews','rm-corrections','rm-audit','marketing','admin','credit-cards']);$audit=$this->tableMetadata('rm_audits');$history=$this->tableMetadata('rm_audit_history');foreach(RmAuditsSchema::AUDIT_COLUMNS as$column)if(!isset($audit['columns'][$column]))throw new MigrationException("Missing column {$column} on rm_audits.");foreach(RmAuditsSchema::HISTORY_COLUMNS as$column)if(!isset($history['columns'][$column]))throw new MigrationException("Missing column {$column} on rm_audit_history.");foreach(['fk_rm_audits_property','fk_rm_audits_submitter','fk_rm_audits_completer']as$key)if(($audit['foreign_keys'][$key]??'')!=='RESTRICT')throw new MigrationException("Invalid foreign key {$key}.");foreach(['fk_rm_audit_history_audit','fk_rm_audit_history_actor']as$key)if(($history['foreign_keys'][$key]??'')!=='RESTRICT')throw new MigrationException("Invalid foreign key {$key}.");if(!isset($audit['checks']['chk_rm_audits_status'],$history['checks']['chk_rm_audit_history_event']))throw new MigrationException('Missing RM Audit lifecycle check.');
    }
    private function verifySupplyOrdersSchema():void{$this->verifyRmAuditsSchema();$orders=$this->tableMetadata('supply_orders');foreach(SupplyOrdersSchema::COLUMNS as$column)if(!isset($orders['columns'][$column]))throw new MigrationException("Missing column {$column} on supply_orders.");foreach(['uq_supply_orders_public_id','idx_supply_orders_property_submitted']as$index)if(!isset($orders['indexes'][$index]))throw new MigrationException("Missing index {$index} on supply_orders.");foreach(['fk_supply_orders_property','fk_supply_orders_submitter']as$key)if(($orders['foreign_keys'][$key]??'')!=='RESTRICT')throw new MigrationException("Invalid foreign key {$key} on supply_orders.");}
    ...
    private function verifyEmployeeEmergencyContactSchema():void
    {
        $metadata=$this->tableMetadata(EmployeeEmergencyContactSchema::TABLE);if(strcasecmp($metadata['engine'],'InnoDB')!==0||strcasecmp($metadata['collation'],'utf8mb4_0900_ai_ci')!==0)throw new MigrationException('employee_emergency_contacts has an invalid engine or collation.');if(array_keys($metadata['columns'])!==EmployeeEmergencyContactSchema::COLUMNS){$actual=array_keys($metadata['columns']);sort($actual);$expected=EmployeeEmergencyContactSchema::COLUMNS;sort($expected);if($actual!==$expected)throw new MigrationException('employee_emergency_contacts has an unexpected column set.');}foreach(EmployeeEmergencyContactSchema::INDEXES as $index)if(!isset($metadata['indexes'][$index]))throw new MigrationException("Missing index {$index} on employee_emergency_contacts.");if(($metadata['foreign_keys'][EmployeeEmergencyContactSchema::FOREIGN_KEY]??'')!=='RESTRICT')throw new MigrationException('Emergency contact employee foreign key must be restrictive.');$s=$this->connection->prepare("SELECT COLUMN_NAME,COLUMN_TYPE,IS_NULLABLE,COLUMN_DEFAULT,EXTRA FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=? AND TABLE_NAME='employee_emergency_contacts'");$database=$this->expectedDatabase;$s->bind_param('s',$database);$s->execute();$columns=[];foreach($s->get_result()->fetch_all(MYSQLI_ASSOC) as $row)$columns[(string)$row['COLUMN_NAME']]=$row;$s->close();if(($columns['alternate_phone']['IS_NULLABLE']??'')!=='YES'||($columns['employee_id']['COLUMN_TYPE']??'')!=='bigint unsigned'||!str_contains(strtoupper((string)($columns['updated_at']['EXTRA']??'')),'ON UPDATE CURRENT_TIMESTAMP'))throw new MigrationException('Emergency contact columns do not match the approved schema.');
    }
    private function verifyPropertyAccessSchema():void
    {
        $metadata=$this->tableMetadata(PropertyAccessSchema::TABLE);if(strcasecmp($metadata['engine'],'InnoDB')!==0||strcasecmp($metadata['collation'],'utf8mb4_0900_ai_ci')!==0)throw new MigrationException('user_property_access has an invalid engine or collation.');$actual=array_keys($metadata['columns']);sort($actual);$expected=PropertyAccessSchema::COLUMNS;sort($expected);if($actual!==$expected)throw new MigrationException('user_property_access has an unexpected column set.');foreach(PropertyAccessSchema::INDEXES as $index)if(!isset($metadata['indexes'][$index]))throw new MigrationException("Missing index {$index} on user_property_access.");foreach(PropertyAccessSchema::FOREIGN_KEYS as $key)if(($metadata['foreign_keys'][$key]??'')!=='RESTRICT')throw new MigrationException("Invalid foreign key {$key} on user_property_access.");
    }
    private function verifyApplicationReviewsSchema():void
    {
        foreach(['application_reviews'=>[ApplicationReviewsSchema::REVIEW_COLUMNS,ApplicationReviewsSchema::REVIEW_INDEXES,ApplicationReviewsSchema::REVIEW_FKS],'application_review_history'=>[ApplicationReviewsSchema::HISTORY_COLUMNS,ApplicationReviewsSchema::HISTORY_INDEXES,ApplicationReviewsSchema::HISTORY_FKS]] as $table=>[$columns,$indexes,$keys]){$metadata=$this->tableMetadata($table);if(strcasecmp($metadata['engine'],'InnoDB')!==0||strcasecmp($metadata['collation'],'utf8mb4_0900_ai_ci')!==0)throw new MigrationException("{$table} has an invalid engine or collation.");$actual=array_keys($metadata['columns']);sort($actual);$expected=$columns;sort($expected);if($actual!==$expected)throw new MigrationException("{$table} has an unexpected column set.");foreach($indexes as $index)if(!isset($metadata['indexes'][$index]))throw new MigrationException("Missing index {$index} on {$table}.");foreach($keys as $key)if(($metadata['foreign_keys'][$key]??'')!=='RESTRICT')throw new MigrationException("Invalid foreign key {$key} on {$table}.");}
        $this->assertCheckValues('application_reviews','chk_application_reviews_status',['pending_review','approved','denied']);
        $this->assertCheckValues('application_review_history','chk_application_review_history_event',['submitted','approved','denied']);
        $this->assertCheckValues('application_review_history','chk_application_review_history_to_status',['pending_review','approved','denied']);
    }

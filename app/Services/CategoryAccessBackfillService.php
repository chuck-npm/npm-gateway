...
final class CategoryAccessBackfillService
{
    ...
    {
        $expected=['chuck'=>array_keys($this->categories),'tim'=>array_values(array_diff(array_keys($this->categories),['admin']))];if(array_keys($this->categories)!==['finance','human-resources','marketing','admin','credit-cards'])throw new \RuntimeException('The approved category allowlist is unavailable or invalid.');$users=[];foreach(array_keys($expected) as $username){$user=$this->store->findUserByUsername($username);if($user===null||$user['status']!=='active')throw new \RuntimeException("Required active Gateway user {$username} is missing, inactive, or ambiguous.");$users[$username]=$user;}

...
final class StorageObjectKeyGenerator
{
 ...
 public function generateCreditCardReceipt(string $prefix,string $mimeType,?\DateTimeImmutable $now=null):string
 {
  $prefix=trim(str_replace('\\','/',$prefix),'/').'/';if(!preg_match('#^[a-z0-9/_-]+/$#D',$prefix))throw new \InvalidArgumentException('Storage prefix is invalid.');$extension=match($mimeType){'application/pdf'=>'pdf','image/jpeg'=>'jpg','image/png'=>'png',default=>throw new \InvalidArgumentException('Receipt MIME type is not approved.')};$utc=($now??new \DateTimeImmutable('now',new \DateTimeZone('UTC')))->setTimezone(new \DateTimeZone('UTC'));return $prefix.$utc->format('Y/m').'/'.strtoupper(bin2hex(random_bytes(16))).'.'.$extension;
 }
}

...
final class CompanyNoticePublicationService
{
 ...
 public function audienceCount():int{return count($this->recipients->eligibleAudience());}
 public function publish(array $review,AuthenticatedUser $actor):array
 {
  ...
  try{
   ...
   $stage='recipient_resolution';$audience=$this->recipients->eligibleAudience();$at=$this->clock->now();$timestamp=$at->format('Y-m-d H:i:s');$public=$this->ids->generate();$this->tx->begin();
   try{

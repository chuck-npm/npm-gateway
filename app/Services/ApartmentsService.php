...
{
 public function __construct(private ApartmentsWorkbookParser$parser,private ApartmentsRepository$repo,private InitializationTransactionInterface$tx,private PublicIdGenerator$ids,private ClockInterface$clock,private AuditService$audit,private ?ApartmentsReportCriteriaFactory$criteriaFactory=null){}
 public function history():array{return$this->repo->history();}
 public function report(array$query):array{$properties=$this->repo->reportProperties();$criteria=($this->criteriaFactory??new ApartmentsReportCriteriaFactory())->fromQuery($query,$properties,$this->repo->mostRecentRange());$report=$criteria->valid()?$this->repo->report($criteria,$properties):['rows'=>array_map(static fn(array$p):array=>$p+['total'=>0,'duration'=>0],$properties),'totals'=>['count'=>0,'duration'=>0],'details'=>[],'page'=>1,'pages'=>1,'per_page'=>$criteria->perPage,'records'=>0,'from'=>0,'to'=>0];return$report+['criteria'=>$criteria,'properties'=>$properties];}

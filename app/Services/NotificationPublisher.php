...
final class NotificationPublisher implements NotificationPublisherInterface
{
 ...
 public function publish(EmployeeAnnouncement $a,int $actorUserId,int $actorEmployeeId,string $actorPublicId):array
 {
  ...
  $resolver=$this->resolver??new NotificationRecipientResolver($this->recipients,new GatewayNotificationRecipientEligibilityPolicy());$at=$this->clock->now()->format('Y-m-d H:i:s');$audience=$resolver->resolveCompanyAudience();$resolver->requireApproved($audience);$this->tx->begin();

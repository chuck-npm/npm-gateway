...
use NpmGateway\Support\{PhoneFormatter,PropertyAddressFormatter};
use NpmGateway\ValueObjects\{PropertyDirectoryCriteria,PropertyDirectoryPage,PropertyDirectoryRow};
final class PropertyQueryService
{
    ...
    {
        $total=$this->properties->countDirectoryResults($criteria);$pages=max(1,(int)ceil($total/$criteria->perPage));$page=min($criteria->page,$pages);if($page!==$criteria->page)$criteria=new PropertyDirectoryCriteria($criteria->search,$criteria->sort,$criteria->direction,$page,$criteria->perPage);
        $rows=array_map(fn(array$r):PropertyDirectoryRow=>new PropertyDirectoryRow((string)($r['public_id']??''),(int)$r['prop_id'],(string)$r['display_name'],$this->addresses->format($r['address_line_1'],$r['city'],$r['state'],$r['postal_code']),$this->phones->format($r['office_phone']),$r['ivr_number']===null?'—':$this->phones->format($r['ivr_number']),(string)$r['manager_name']),$this->properties->searchDirectory($criteria));

...
final class PropertyAccessRepository implements PropertyAccessStoreInterface
{
 ...
 public function accessibleActiveCommunities(int $userId):array{$s=$this->db->prepare("SELECT p.id,p.public_id,p.slug,p.property_code,p.display_name FROM user_property_access a JOIN users u ON u.id=a.user_id JOIN properties p ON p.id=a.property_id WHERE a.user_id=? AND u.status='active' AND p.status='active' AND p.property_code<>'CO' ORDER BY p.display_name,p.id");$s->bind_param('i',$userId);$s->execute();$rows=$s->get_result()->fetch_all(MYSQLI_ASSOC);$s->close();return $rows;}
 public function activeCommunityBySlug(string $slug):?array{$s=$this->db->prepare("SELECT id,public_id,slug,property_code,display_name FROM properties WHERE slug=? AND status='active' AND property_code<>'CO' LIMIT 2");$s->bind_param('s',$slug);$s->execute();$rows=$s->get_result()->fetch_all(MYSQLI_ASSOC);$s->close();return count($rows)===1?$rows[0]:null;}

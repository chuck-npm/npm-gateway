...
final class UserRepository implements UserStoreInterface
{
    ...
    {
        $statement=$this->connection->prepare('SELECT u.id,u.public_id,u.employee_id,u.username,u.password_hash,u.status,u.failed_login_count,u.locked_until,e.public_id employee_public_id,e.first_name,e.last_name,e.employment_status FROM users u JOIN employees e ON e.id=u.employee_id WHERE u.username=? LIMIT 1');
    ...
    {
        $statement=$this->connection->prepare("SELECT u.id,u.public_id,u.employee_id,u.username,u.status,e.public_id employee_public_id,e.first_name,e.last_name,e.employment_status FROM users u JOIN employees e ON e.id=u.employee_id WHERE u.id=? LIMIT 1");

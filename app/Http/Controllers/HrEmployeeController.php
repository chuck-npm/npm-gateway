...
final class HrEmployeeController
{
    ...
    {
        if(!$this->access->allows($context))return new Response(403,'Forbidden');$user=$context->user;$logoutCsrfToken=$this->csrf->token();$directoryPage=$this->directory->search($this->criteria->fromQuery($request->query));$success=(string)$this->flash->pull('hr_employee_success','');$showCorporateTools=true;$navbarCorporateItems=$this->tools->tools();ob_start();require $this->views.'/human-resources/employees/index.php';return new Response(200,(string)ob_get_clean());
    }
    public function create(AuthenticatedRequestContext $context):Response
    {
        if(!$this->access->allows($context))return new Response(403,'Forbidden');$user=$context->user;$logoutCsrfToken=$this->csrf->token();$csrfToken=$this->csrf->token();$errors=(array)$this->flash->pull('hr_employee_errors',[]);$input=(array)$this->flash->pull('hr_employee_input',[]);if(!isset($input['start_date']))$input['start_date']=$this->creation->currentDate();$properties=$this->creation->eligibleProperties();$showCorporateTools=true;$navbarCorporateItems=$this->tools->tools();ob_start();require $this->views.'/human-resources/employees/create.php';return new Response(200,(string)ob_get_clean());
    }

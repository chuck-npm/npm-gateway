...
{
 public function __construct(private readonly NotificationQueryService $queries,private readonly NotificationAcknowledgmentService $ack,private readonly NotificationPresentationService $presentation,private readonly CorporateAccessService $access,private readonly CorporateToolsProviderInterface $tools,private readonly CsrfService $csrf,private readonly FlashSession $flash,private readonly string $views){}
 public function index(Request $request,AuthenticatedRequestContext $context):Response{$user=$context->user;$requested=(string)($request->query['view']??'');$filter=in_array($requested,['acknowledged','informational'],true)?$requested:'outstanding';$notices=$this->queries->listing($user->id,$filter);$notificationCount=$this->queries->count($user->id);$csrfToken=$this->csrf->token();$logoutCsrfToken=$csrfToken;$navbarCorporateItems=$this->tools->tools($context);$canCreateNotice=$this->access->canAccessCategory($context,'company-notices');$success=(string)$this->flash->pull('notification_success','');ob_start();require $this->views.'/notifications/index.php';return new Response(200,(string)ob_get_clean());}

...
final class PropertyWorkspaceController
{
    ...
    public function hrDirectory(Request $request,AuthenticatedRequestContext $context):Response{return $this->access->allows($context)?$this->renderDirectory($request,$context,true):new Response(403,'Forbidden');}
    public function create(AuthenticatedRequestContext $context):Response
    {
        if(!$this->access->allows($context))return new Response(403,'Forbidden');$user=$context->user;$logoutCsrfToken=$this->csrf->token();$csrfToken=$this->csrf->token();$errors=(array)$this->flash->pull('property_errors',[]);$input=(array)$this->flash->pull('property_input',[]);$showCorporateTools=true;$navbarCorporateItems=$this->corporateTools->tools();ob_start();require $this->views.'/human-resources/properties/create.php';return new Response(200,(string)ob_get_clean());
    }
    public function store(Request $request,AuthenticatedRequestContext $context):Response
    {
        if(!$this->access->allows($context))return new Response(403,'Forbidden');if(!$this->csrf->valid($request->post['_token']??null))return new Response(419,'Invalid request.');
        try{$this->administration->create($request->post,$context->user);$this->flash->put('success','Property added successfully.');}
        catch(InvalidPropertyDataException $e){$this->flash->put('property_errors',$e->errors);$this->flash->put('property_input',$e->input);return Response::redirect('/human-resources/properties/create');}
        return Response::redirect('/human-resources/properties');
    }
    private function renderDirectory(Request $request,AuthenticatedRequestContext $context,bool $hr):Response
    {
        $user=$context->user;$logoutCsrfToken=$this->csrf->token();$directoryPage=$this->directory->search($this->criteria->fromQuery($request->query));$success=$hr?(string)$this->flash->pull('success',''):'';$showCorporateTools=$this->access->allows($context);$navbarCorporateItems=$showCorporateTools?$this->corporateTools->tools():[];ob_start();require $this->views.'/properties/index.php';return new Response(200,(string)ob_get_clean());
    }
}

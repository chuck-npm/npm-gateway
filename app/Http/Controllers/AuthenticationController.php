...
final class AuthenticationController
{
 ...
 public function loginForm(?string $error=null,string $username=''):Response{return new Response(200,$this->render('auth/login.php',['csrfToken'=>$this->csrf->token(),'error'=>$error,'username'=>$username]),['Cache-Control'=>'private, no-store']);}
 public function login(Request $request,\DateTimeImmutable $now):Response
 {
  ...
  catch(InvalidCredentialsException){return $this->loginForm('The username or password is incorrect.',(string)($request->post['username']??''));}
 }

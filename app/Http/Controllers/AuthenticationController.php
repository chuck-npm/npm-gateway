...
{
 public function __construct(private readonly AuthenticationServiceInterface $authentication,private readonly SessionServiceInterface $sessions,private readonly SessionCookie $cookie,private readonly CsrfService $csrf,private readonly string $views){}
 ...
 public function login(Request $request,\DateTimeImmutable $now):Response
 {
  ...
  try{$result=$this->authentication->authenticate(new LoginRequest($request->post['username']??'',$request->post['password']??''),new ClientContext($request->ip(),$request->agent(),$now));$r=Response::redirect('/dashboard');return new Response($r->status,'',$r->headers,[$this->cookie->set($result->session->reveal())]);}

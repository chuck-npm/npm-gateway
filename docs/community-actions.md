...
`CommunityActionProvider` is the single catalog, in order: Application Reviews, Credit Card Purchases, RM Corrections, Renovation Request, Request Appliances, Appliance Distribution, HVAC Service Request, Order Supplies, Eviction Checks, and RM Audit. Order Supplies is an implemented property-scoped immutable request history; planned cards remain visibly unavailable. RM means Rent Manager and is not expanded in interface labels.

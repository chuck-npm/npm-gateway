...
final class AuthenticationWorkflowTest extends TestCase
{
 ...
 public function testFailedLoginIsNeutralAndNeverEchoesPassword():void{$this->authentication->fail=true;$r=$this->kernel->handle($this->loginRequest('TEST-secret-password'),$this->now());self::assertSame(200,$r->status);self::assertStringContainsString('The username or password is incorrect.',$r->body);self::assertStringContainsString('value="admin"',$r->body);self::assertStringNotContainsString('TEST-secret-password',$r->body);self::assertSame([],$r->cookies);}

    ...
    public function testBootstrapInDisposableDatabase(): void
    {
        ...
        try {
            ...
            $service = new SystemInitializationService(
                ...
                self::assertSame(count($categoryConfig['categories']), self::rowCount($connection, 'user_category_access'));
                ...
                self::assertCount(8,$home->universalTools);
                self::assertCount(count($categoryConfig['categories']),$home->corporateTools);
                self::assertCount(3,array_filter($home->universalTools,static fn($tool):bool=>$tool->enabled));self::assertSame('/employees',$home->universalTools[0]->route);self::assertSame('/properties',$home->universalTools[1]->route);self::assertSame('/notifications',$home->universalTools[3]->route);
                foreach($home->universalTools as $tool){if(in_array($tool->key,['employee-directory','property-information','notifications'],true))continue;self::assertFalse($tool->enabled);self::assertNull($tool->route);}
                self::assertCount(5,array_filter($home->corporateTools,static fn($tool):bool=>$tool->enabled));$corporateTools=[];foreach($home->corporateTools as $tool)$corporateTools[$tool->key]=$tool;self::assertSame('/human-resources',$corporateTools['human-resources']->route);self::assertSame('/company-notices',$corporateTools['company-notices']->route);self::assertSame('/admin',$corporateTools['admin']->route);
                ...
                self::assertSame(200,$response->status);self::assertStringContainsString('Corporate Tools',$response->body);self::assertStringContainsString('aria-label="Corporate tools menu"',$response->body);self::assertSame(2,substr_count($response->body,'gateway-navbar__disabled-item'));self::assertStringNotContainsString('href="#"',$response->body);
                ...
                self::assertCount(8,$managerHome->universalTools);self::assertCount(count($categoryConfig['categories']),$managerHome->corporateTools);

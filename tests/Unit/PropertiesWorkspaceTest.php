...
final class PropertiesWorkspaceTest extends TestCase
{
    ...
    public function testDirectoryDefaultsToAlphabeticalNameWithDeterministicIdOrdering():void
    {
        $criteria=new \NpmGateway\ValueObjects\PropertyDirectoryCriteria();self::assertSame('name',$criteria->sort);self::assertSame('asc',$criteria->direction);$repository=(string)file_get_contents(dirname(__DIR__,2).'/app/Repositories/PropertyRepository.php');self::assertStringContainsString("'name'=>'p.display_name'",$repository);self::assertStringContainsString('ORDER BY {$order} {$direction},p.id ASC',$repository);
    }
    public function testEditUniquenessChecksExcludeCurrentRecord():void
    {
        $store=new class implements PropertyStoreInterface{public array$exclusions=[];public function propIdExists(int$id,?int$excludeId=null):bool{$this->exclusions[]=$excludeId;return false;}public function propertyCodeExists(string$code,?int$excludeId=null):bool{$this->exclusions[]=$excludeId;return false;}public function slugExists(string$slug,?int$excludeId=null):bool{$this->exclusions[]=$excludeId;return false;}public function findByPublicId(string$publicId):?array{return null;}public function insert(array$property):int{return 1;}public function update(int$id,array$property):void{}};[, $errors]=(new PropertyValidator($store,new PhoneFormatter()))->validate($this->valid(),77);self::assertSame([],$errors);self::assertSame([77,77,77],$store->exclusions);
    }
    ...
    private function store(?int $prop=null,?string $code=null,?string $slug=null):PropertyStoreInterface{return new class($prop,$code,$slug) implements PropertyStoreInterface{public function __construct(private ?int $prop,private ?string $code,private ?string$slug){}public function propIdExists(int$id,?int$excludeId=null):bool{return$id===$this->prop;}public function propertyCodeExists(string$code,?int$excludeId=null):bool{return$code===$this->code;}public function slugExists(string$slug,?int$excludeId=null):bool{return$slug===$this->slug;}public function findByPublicId(string$publicId):?array{return null;}public function insert(array$property):int{return 1;}public function update(int$id,array$property):void{}};}
}

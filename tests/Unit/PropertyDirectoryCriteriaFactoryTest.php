...
final class PropertyDirectoryCriteriaFactoryTest extends TestCase
{
    public function testEmptyQueryUsesSafeExplicitDefaultsWithoutWarnings():void
    {
        ...
        self::assertSame([],$warnings);self::assertSame('',$criteria->search);self::assertSame('name',$criteria->sort);self::assertSame('asc',$criteria->direction);self::assertSame(1,$criteria->page);self::assertSame(25,$criteria->perPage);
    }
    ...
    public function testEditGetPostCsrfAuthorizationAndSuccessFlash():void
    {
        $controller=$this->controller();$authorized=$this->context('listed');$public=str_repeat('P',26);$page=$controller->edit($public,$authorized);self::assertSame(200,$page->status);foreach(['Edit Pine Manor','value="1157"','value="PM"','value="pine-manor"','Save Changes','>Cancel</a>']as$text)self::assertStringContainsString($text,$page->body);self::assertStringContainsString('/human-resources/properties/'.$public.'/edit',$page->body);self::assertStringNotContainsString('name="id"',$page->body);self::assertStringNotContainsString('name="public_id"',$page->body);preg_match('/name="_token" value="([a-f0-9]+)"/',$page->body,$match);self::assertSame(419,$controller->update(new Request('POST','/human-resources/properties/'.$public.'/edit',['_token'=>'bad']),$public,$authorized)->status);$post=['_token'=>$match[1],'prop_id'=>'1157','property_code'=>'PM','display_name'=>'Pine Manor Updated','slug'=>'pine-manor','status'=>'active','address_line_1'=>'2 Pine Way','city'=>'Columbus','state'=>'GA','postal_code'=>'31904','timezone'=>'America/New_York','office_phone'=>'(706) 555-0101','manager_email'=>'shared@example.test','website_url'=>'https://example.test','ivr_number'=>'(706) 555-0102','ivr_routing_email'=>'1157@rentertext.com'];$response=$controller->update(new Request('POST','/human-resources/properties/'.$public.'/edit',$post),$public,$authorized);self::assertSame(303,$response->status);self::assertSame('/human-resources/properties',$response->headers['Location']);self::assertStringContainsString('Property updated successfully.',$controller->hrDirectory(new Request('GET','/human-resources/properties'),$authorized)->body);self::assertSame(403,$controller->edit($public,$this->context('unlisted'))->status);self::assertSame(403,$controller->update(new Request('POST','/human-resources/properties/'.$public.'/edit',[]),$public,$this->context('unlisted'))->status);self::assertSame(404,$controller->edit(str_repeat('Z',26),$authorized)->status);
    }
    private function controller():PropertyWorkspaceController
    {
        $store=new class implements PropertyStoreInterface,PropertyDirectoryStoreInterface{public array$property=['id'=>7,'public_id'=>'PPPPPPPPPPPPPPPPPPPPPPPPPP','prop_id'=>1157,'property_code'=>'PM','display_name'=>'Pine Manor','slug'=>'pine-manor','status'=>'active','address_line_1'=>'2 Pine Way','city'=>'Columbus','state'=>'GA','postal_code'=>'31904','timezone'=>'America/New_York','office_phone'=>'+17065550101','manager_email'=>'shared@example.test','website_url'=>'https://example.test','ivr_number'=>'+17065550102','ivr_routing_email'=>'1157@rentertext.com'];public function propIdExists(int$id,?int$excludeId=null):bool{return false;}public function propertyCodeExists(string$code,?int$excludeId=null):bool{return false;}public function slugExists(string$slug,?int$excludeId=null):bool{return false;}public function findByPublicId(string$publicId):?array{return$publicId===$this->property['public_id']?$this->property:null;}public function insert(array$property):int{return 1;}public function update(int$id,array$property):void{$this->property=array_replace($this->property,$property);}public function searchDirectory(\NpmGateway\ValueObjects\PropertyDirectoryCriteria$criteria):array{return [];}public function countDirectoryResults(\NpmGateway\ValueObjects\PropertyDirectoryCriteria$criteria):int{return 0;}};

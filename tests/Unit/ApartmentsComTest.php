...
use NpmGateway\Services\ApartmentsReportCriteriaFactory;
...
final class ApartmentsComTest extends TestCase
{
 ...
 public function testRoutesSchemaAndAuthorizationContract():void{$root=dirname(__DIR__,2);$routes=require$root.'/routes/web.php';foreach(['/admin/apartments','/admin/apartments/upload','/admin/apartments/import-history']as$route)self::assertSame(['authentication','protected-principal'],$routes[$route]['middleware']);$migration=(string)file_get_contents($root.'/database/migrations/202608150026_apartments_com_imports.php');foreach(ApartmentsComSchema::TABLES as$table)self::assertStringContainsString('CREATE TABLE '.$table,$migration);foreach(['file_sha256','property_id,occurred_at','call_duration_seconds INT UNSIGNED','ON DELETE RESTRICT']as$text)self::assertStringContainsString($text,$migration);}
 public function testReportCriteriaDefaultsToCallsAndMostRecentSourceRange():void{$properties=$this->properties();$criteria=(new ApartmentsReportCriteriaFactory())->fromQuery([],$properties,['source_started_at'=>'2026-08-01 01:02:03','source_ended_at'=>'2026-08-07 23:59:59']);self::assertSame('calls',$criteria->view);self::assertSame('2026-08-01',$criteria->fromDate);self::assertSame('2026-08-07',$criteria->toDate);self::assertSame('2026-08-08',$criteria->toExclusive);self::assertTrue($criteria->valid());}
 public function testReportCriteriaValidatesDatesPropertyAndPreservesEmailView():void{$properties=$this->properties();$factory=new ApartmentsReportCriteriaFactory();$valid=$factory->fromQuery(['view'=>'email-leads','from'=>'2026-08-01','to'=>'2026-08-07','property'=>'PUBLICBT'],$properties,null);self::assertSame('email-leads',$valid->view);self::assertSame('PUBLICBT',$valid->propertyPublicId);self::assertTrue($valid->valid());$bad=$factory->fromQuery(['from'=>'2026-08-08','to'=>'2026-08-07','property'=>'SUBURBAN'],$properties,null);self::assertSame('From Date must be on or before To Date.',$bad->errors['date_range']);self::assertSame('Choose an Apartments.com property.',$bad->errors['property']);}
 public function testCanonicalReportRosterOrderIncludesZerosAndExcludesSuburban():void
 {
  self::assertSame(['BT','CF','FF','HR','MW','PP','PH','PM','SM','WP'],ApartmentsRepository::OWNER_PROPERTY_CODES);
  self::assertNotContains('SUB',ApartmentsRepository::OWNER_PROPERTY_CODES);
  $root=dirname(__DIR__,2);$view=(string)file_get_contents($root.'/resources/views/admin/apartments/index.php');$service=(string)file_get_contents($root.'/app/Services/ApartmentsService.php');$repository=(string)file_get_contents($root.'/app/Repositories/ApartmentsRepository.php');
  self::assertStringContainsString("['total'=>0",$service);self::assertStringContainsString('TOTAL',$view);self::assertStringContainsString('Total Call Duration',$view);self::assertStringContainsString('Total Email Leads',$view);self::assertStringNotContainsString('35',$repository);
 }
 ...
 private function properties():array{return[['public_id'=>'PUBLICBT','display_name'=>'Boulder Trails']];}

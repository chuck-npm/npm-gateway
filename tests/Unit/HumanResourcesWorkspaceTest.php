...
final class HumanResourcesWorkspaceTest extends TestCase
{
    ...
    {
        $session=[];$access=$this->access(['human-resources']);$controller=new HumanResourcesController($access,new CorporateToolsProvider($access),new CsrfService($session),dirname(__DIR__,2).'/resources/views');$response=$controller->index($this->context('listed'));self::assertSame(200,$response->status);self::assertSame(3,substr_count($response->body,'data-tool-key='));$employees=strpos($response->body,'data-tool-key="employees"');$properties=strpos($response->body,'data-tool-key="properties"');$cards=strpos($response->body,'data-tool-key="credit-cards"');self::assertLessThan($properties,$employees);self::assertLessThan($cards,$properties);self::assertStringContainsString('href="/human-resources/employees"',$response->body);self::assertStringContainsString('href="/human-resources/properties"',$response->body);self::assertStringNotContainsString('Appliances',$response->body);$credit=substr($response->body,(int)$cards,800);self::assertStringNotContainsString('href=',$credit);
    }
